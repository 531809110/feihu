...
//启动服务器app.js/启动数据库
//打开浏览器
//http://127.0.0.1:8080/register?uname=dingding&upwd=123456&email=123456@qq.com&tuijianren=88888888
...
server.get("/register", (req, res) => {
  ...
  pool.query(sql, [u], (err, result) => {
    ...
    if (result.length > 0) {
      ...
      //用户名不存在，可用，写入数据库
      var sql = "INSERT INTO fh_user (uname,upwd,email,tuijianID) VALUES (?,?,?,?)";
      pool.query(sql, [u, p, e, t], (err, result) => {
        if (err) throw err;
        //6.5:将结果返回网页
